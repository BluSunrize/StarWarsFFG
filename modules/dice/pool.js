...
export class DicePoolFFG {
  ...
  renderDiceExpression() {
    let dicePool = [
      Math.max(0, this.proficiency) + "dp",
      Math.max(0, this.ability) + "da",
      Math.max(0, this.challenge) + "dc",
      Math.max(0, this.difficulty) + "di",
      Math.max(0, this.boost) + "db",
      Math.max(0, this.setback) + "ds",
      Math.max(0, this.force) + "df"
    ];
